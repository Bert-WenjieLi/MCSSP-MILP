...
for r in range(1,Requests):
    for k in range(1,TCpoint):
        co.addConstr(quicksum(ReSe[r,s,k] for s in range(1,Sequence)) == ReAs[r,k])
##########
#for r in range(1,Requests):
#    for s in range(1,Sequence):
#        for k in range(1,TCpoint):
        ...
        for i in range(1,Spoint):
            for j in range(1,Dpoint):
                for o in range(1,Dpoint):
                    co.addConstr(2-DeSe[s,j,k]-SuDeNoM[s+1,i,o,k]>=1-DeSu[s+1,j,i,k])
    ...
    for i in range(1,Spoint):
        for j in range(1,Dpoint):
            for o in range(1,Dpoint):
                for k in range(1,TCpoint):
                    if s==1:
                        co.addConstr(2-SuDeNoM[s,i,o,k]-DeSe_1[j,k]>=1-DeSu[s,j,i,k])
...
for k in range(1,TCpoint):
    for s in range(1,Sequence):
        ...
        #start time
            ...
            if s!=1:
                if q==1:
                    timeline[k,s,q]=timeline[k,s-1,q+1]+quicksum((Time_sd_run[i,j,k]+1)*SuDeNoM[s-1,i,j,k] for i in range(1,Spoint) for j in range(1,Dpoint)) 
                if q==2:
                    timeline[k,s,q]=timeline[k,s,q-1]+quicksum((Time_sd_run[i,j,k]+1)*DeSu[s,j,i,k] for i in range(1,Spoint) for j in range(1,Dpoint))
Time_In={}
Time_Out={}
        ...
        for q in range(1,Stages):
            if q==1:
                if s==1:
                    ...
                    Time_In[k,s,q,o] = timeline[k,s,q] + quicksum(DeSu[s,j,i,k]*(1-Path_C[j,o])*(100*(1-Path[i,j,k,o])+(Path_A[i,j,k,o,p]*Time_DC[p,j,k])+(Path_A[i,j,k,o,p+1]*Time_DC[p+1,j,k]))for i in range(1,Spoint) for j in range(1,Dpoint)) - quicksum(SuDeNoM[s-1,i,j,k]*Path_C[j,o]*((Path_A[i,j,k,o,p]*Time_DC[p,j,k])+(Path_A[i,j,k,o,p+1]*Time_DC[p+1,j,k])+1) for i in range(1,Spoint) for j in range(1,Dpoint))
                    Time_Out[k,s,q,o] = Time_In[k,s,q,o] + quicksum(DeSu[s,j,i,k]*(1-Path_C[j,o])*(Path[i,j,k,o]*Time_AB[k,o])for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(DeSu[s,j,i,k]*Path_C[j,o]*((Path_A[i,j,k,o,p]*Time_DC[p,j,k])+(Path_A[i,j,k,o,p+1]*Time_DC[p+1,j,k]))for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(SuDeNoM[s-1,i,j,k]*Path_C[j,o]*((Path_A[i,j,k,o,p]*Time_DC[p,j,k])+(Path_A[i,j,k,o,p+1]*Time_DC[p+1,j,k])+1) for i in range(1,Spoint) for j in range(1,Dpoint))
            else:
                Time_In[k,s,q,o] = timeline[k,s,q] + quicksum(SuDeNoM[s,i,j,k]*(100*(1-Path[i,j,k,o])+(Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k])) for i in range(1,Spoint) for j in range(1,Dpoint))
                if s==Sequence-1:
                    Time_Out[k,s,q,o] = Time_In[k,s,q,o] + quicksum(SuDeNoM[s,i,j,k]*((1-Path_C[j,o])*Path[i,j,k,o]*Time_AB[k,o] + 50*Path_C[j,o]) for i in range(1,Spoint) for j in range(1,Dpoint))
                else:
                    Time_Out[k,s,q,o] = Time_In[k,s,q,o] + quicksum(SuDeNoM[s,i,j,k]*((1-Path_C[j,o])*Path[i,j,k,o]*Time_AB[k,o]) for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(SuDeNoM[s,i,j,k]*(Path_C[j,o]*((Path_A[i,j,k,o,p]*Time_DC[p,j,k])+(Path_A[i,j,k,o,p+1]*Time_DC[p+1,j,k])+1)) for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(DeSu[s+1,j,i,k]*(Path_C[j,o]*((Path_A[i,j,k,o,p]*Time_DC[p,j,k])+(Path_A[i,j,k,o,p+1]*Time_DC[p+1,j,k])))  for i in range(1,Spoint) for j in range(1,Dpoint))
    ...
    for s in range(1,Sequence):
        for q in range(1,Stages):
            if q==1:
                ...
                Time_Out[k,s,q,o] = Time_In[k,s,q,o] + quicksum(DeSu[s,j,i,k]*(Path_D[i,o]*((Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k])+1))for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(DeSu[s,j,i,k]*(1-Path_D[i,o])*Path[i,j,k,o]*Time_AB[k,o]for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(SuDeNoM[s,i,j,k]*Path_D[i,o]*((Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k])) for i in range(1,Spoint) for j in range(1,Dpoint))
            else:
                Time_In[k,s,q,o] = timeline[k,s,q] + quicksum(SuDeNoM[s,i,j,k]*(1-Path_D[i,o])*(100*(1-Path[i,j,k,o])+(Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k])) for i in range(1,Spoint) for j in range(1,Dpoint)) - quicksum(DeSu[s,j,i,k]*Path_D[i,o]*((Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k])+1)for i in range(1,Spoint) for j in range(1,Dpoint))
                Time_Out[k,s,q,o] = Time_In[k,s,q,o] + quicksum(SuDeNoM[s,i,j,k]*((1-Path_D[i,o])*Path[i,j,k,o]*Time_AB[k,o]) for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(SuDeNoM[s,i,j,k]*Path_D[i,o]*((Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k]))for i in range(1,Spoint) for j in range(1,Dpoint)) + quicksum(DeSu[s,j,i,k]*Path_D[i,o]*((Path_A[i,j,k,o,p]*Time_SC[p,i,k])+(Path_A[i,j,k,o,p+1]*Time_SC[p+1,i,k])+1)for i in range(1,Spoint) for j in range(1,Dpoint))
            ...
            for u in range(1,Sequence):
               for h in range(1,Stages):
                    for o in range(1,OverZone):
                        if k==1:
                            if o==1:
                                Time_wait[k,s,q,u,h,o]=Time_Out[k+1,u,h,o]-Time_In[k,s,q,o] 
                                Time_wait[k+1,u,h,s,q,o]=Time_Out[k,s,q,o]-Time_In[k+1,u,h,o] 
                        if k==1:
                            if o==2:
                                Time_wait[k,s,q,u,h,o]=Time_Out[k+2,u,h,o]-Time_In[k,s,q,o] 
                                Time_wait[k+2,u,h,s,q,o]=Time_Out[k,s,q,o]-Time_In[k+2,u,h,o] 
                        if k==2:
                            if o==3:
                                Time_wait[k,s,q,u,h,o]=Time_Out[k+1,u,h,o]-Time_In[k,s,q,o] 
                                Time_wait[k+1,u,h,s,q,o]=Time_Out[k,s,q,o]-Time_In[k+1,u,h,o] 
                    ...
                    for o in range(1,OverZone):                        
                        ...
                        if k==1 or k==2:
                            if o==1:
                                ...
                                co.addConstr(Time_wait[k,s,q,u,h,o]<=300*judge[k,s,q,u,h,o])
                        if k==1 or k==3:
                            if o==2:
                                co.addConstr(Time_wait[k,s,q,u,h,o]>=(judge[k,s,q,u,h,o]-1)*300)
                                co.addConstr(Time_wait[k,s,q,u,h,o]<=300*judge[k,s,q,u,h,o])        
...
time={}
for k in range(1,TCpoint):  
    DST[k]=quicksum((Time_sd_run[i,j,k]+1)*DeSu[s,j,i,k] for s in range(1,Sequence) for i in range(1,Spoint) for j in range(1,Dpoint))
    SDT[k]=quicksum((Time_sd_run[i,j,k]+1)*SuDeNoM[s,i,j,k] for i in range(1,Spoint) for j in range(1,Dpoint) for s in range(1,Sequence))
    time[k]=SDT[k]+DST[k]
Time = quicksum(time[k] for k in range(1,TCpoint))

co.Params.IntFeasTol = 1e-3
...
if co.status==GRB.status.OPTIMAL or co.status==GRB.status.TIME_LIMIT:
    print ('\nThe best solution time is',' %.2f' % (co.objval/100),'unit time')
    #print ('---------------------') 
    #for r in range(1,Requests):
    #    for i in range(1,Spoint):
    #        for j in range(1,Dpoint):
            ...
            for j in range(1,Dpoint):
                for i in range(1,Spoint):
                    if SuDeNoM[s,i,j,k].x > 0.99:
                        ...
                        print ('[',s,i,j,k,']','%.2f' %Time_sd_run[i,j,k])
            ...
            for j in range(1,Dpoint):
                for i in range(1,Spoint):
                    if DeSu[s,j,i,k].x > 0.99:
                        ...
                        #print  Time_sd_run[i,j,k]
#    print ('---------------------')
#    print('Tower crane, Sequence, Stage') 

#    for s in range(1,Sequence):
#        for k in range(1,TCpoint):
#            for q in range(1,Stages):
#                print ('[',k,s,q,']', timeline[k,s,q].getValue())
...
#                                    print(k+1,u,h,s,q,o,Time_wait[k+1,u,h,s,q,o].getValue(),judge[k+1,u,h,s,q,o].x)
#                            if k==1:
#                                if o==2:
